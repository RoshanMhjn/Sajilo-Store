...
router.get("/ai/insights", async (_req, res) => {
  ...
  const thisWeekSales = sales.filter(
    (s) => new Date(s.createdAt) >= thisWeekStart,
  );
  const lastWeekSales = sales.filter(
    (s) =>
      new Date(s.createdAt) >= lastWeekStart &&
  ...
  const insights = [
    {
      ...
      title:
        revenueChange >= 0
          ? `Revenue up ${Math.abs(revenueChange).toFixed(1)}% this week`
          : `Revenue down ${Math.abs(revenueChange).toFixed(1)}% this week`,
    ...
    },
    {
      ...
      description: `Store processed ${thisWeekSales.length} sales transactions this week generating रू ${thisWeekRevenue.toFixed(2)} in revenue. Average order value: रू ${thisWeekSales.length > 0 ? (thisWeekRevenue / thisWeekSales.length).toFixed(2) : "0"}.`,
      severity: "info",
      metric: "transactions",
  ...
  if (productSales.size > 0) {
    const topEntry = Array.from(productSales.entries()).sort(
      (a, b) => b[1] - a[1],
    )[0];
...
router.get("/ai/forecast", async (_req, res) => {
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    const revenue =
      avgDaily * trend * dayMultiplier * (0.85 + Math.random() * 0.3);
    forecastPoints.push({
      date: date.toISOString().slice(0, 10),
      revenue: parseFloat(revenue.toFixed(2)),
      sales: Math.round(revenue / 25),
    });
  }

  const nextWeekRevenue = forecastPoints
    .slice(0, 7)
  ...
  res.json({
    ...
    seasonalTrends: [
      ...
      "Month-end spike expected",
      "Holiday promotions recommended",
    ],
  });
});
  ...
  const slowMoving = products
    .map((p) => {
      ...
      return {
        ...
        recommendation:
          days > 60 ? "clearance" : days > 30 ? "discount" : "promote",
        suggestedDiscount: days > 60 ? 30 : days > 30 ? 15 : 10,
      };
    })
...
router.post("/ai/chat", async (req, res) => {
  ...
  if (
    ...
    (lowerMsg.includes("sale") || lowerMsg.includes("revenue"))
  ) {
    response = `Today's performance: ${todaySales.length} transactions totaling रू ${todayRevenue.toFixed(2)} in revenue. Average order value: रू ${todaySales.length > 0 ? (todayRevenue / todaySales.length).toFixed(2) : "0"}.`;
  } else if (lowerMsg.includes("low") && lowerMsg.includes("stock")) {
    const lowStock = products.filter(
      (p) => (invMap.get(p.id) ?? 0) <= Number(p.minStock),
    );
    response = `Currently ${lowStock.length} products are below minimum stock levels: ${lowStock
      .slice(0, 5)
      .map((p) => `${p.name} (${invMap.get(p.id) ?? 0} ${p.unit})`)
      .join(
        ", ",
      )}${lowStock.length > 5 ? ` and ${lowStock.length - 5} more` : ""}.`;
  } else if (
    lowerMsg.includes("top") &&
    (lowerMsg.includes("product") || lowerMsg.includes("selling"))
  ) {
    const productSales = new Map<number, number>();
    ...
    const monthSales = sales.filter(
      (s) => new Date(s.createdAt) >= monthStart && s.status === "completed",
    );
    const monthRevenue = monthSales.reduce(
      (s, sale) => s + Number(sale.total),
      0,
    );
    response = `This month's revenue: रू ${monthRevenue.toFixed(2)} from ${monthSales.length} transactions. Today's revenue: रू ${todayRevenue.toFixed(2)}.`;
  } else {
    response = `I can help you with store analytics. Based on your current data: ${products.length} products, ${customers.length} customers, ${employees.filter((e) => e.status === "active").length} active staff, and रू ${todayRevenue.toFixed(2)} in today's revenue. What specific insights would you like?`;
  }
