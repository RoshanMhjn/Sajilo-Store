...
import { Search, Plus, Minus, Trash2, ShoppingCart, CreditCard, Banknote, Smartphone, CheckCircle2, X, Printer, Star } from "lucide-react";
...
const tierConfig: Record<string, { label: string; discount: number; color: string; next?: string; nextAt?: number }> = {
  basic:    { label: "Basic",    discount: 0,  color: "bg-gray-100 text-gray-700",   next: "Gold",     nextAt: 10000 },
  gold:     { label: "Gold",     discount: 5,  color: "bg-yellow-100 text-yellow-800", next: "Platinum", nextAt: 50000 },
  platinum: { label: "Platinum", discount: 10, color: "bg-blue-100 text-blue-800" },
};
...
export default function POS() {
  ...
  const selectedCustomer = customerList.find((c: any) => c.id === customerId) ?? null;
  const tierInfo = selectedCustomer ? (tierConfig[selectedCustomer.membershipTier] ?? tierConfig.basic) : null;
  ...
  async function handleCheckout() {
    ...
    try {
      ...
      setLastSale({ ...sale, customerName: selectedCustomer?.name, memberNumber: selectedCustomer?.memberNumber, tier: selectedCustomer?.membershipTier });
      setReceiptOpen(true);
      clearCart();
    ...
    <>
      ...
      {lastSale && (
        <div className="print-receipt p-4 font-mono text-xs">
          <div className="text-center font-bold text-base mb-1">Supermarket OS</div>
          <div className="text-center text-xs mb-1">Kathmandu, Nepal | Tel: 01-4XXXXXX</div>
          ...
          {lastSale.customerName && (
            <div className="mb-1">
              ...
              <div>Tier: {tierConfig[lastSale.tier ?? "basic"]?.label ?? "Basic"}</div>
            </div>
          )}
          ...
          <div className="border-t border-dashed pt-1">
            <div className="flex justify-between"><span>Subtotal</span><span>रू{Number(lastSale.subtotal).toFixed(2)}</span></div>
            {Number(lastSale.discount) > 0 && <div className="flex justify-between"><span>Tier Discount ({Number(lastSale.tierDiscountPct).toFixed(0)}%)</span><span>-रू{Number(lastSale.discount).toFixed(2)}</span></div>}
            <div className="flex justify-between"><span>Tax</span><span>रू{Number(lastSale.tax).toFixed(2)}</span></div>
            <div className="flex justify-between font-bold"><span>TOTAL</span><span>रू{Number(lastSale.total).toFixed(2)}</span></div>
            <div className="flex justify-between"><span>Paid ({lastSale.paymentMethod})</span><span>रू{Number(lastSale.amountPaid).toFixed(2)}</span></div>
            {Number(lastSale.change) > 0 && <div className="flex justify-between"><span>Change</span><span>रू{Number(lastSale.change).toFixed(2)}</span></div>}
          </div>
          ...
          <div className="text-center mt-2 text-xs">Thank you for shopping!<br/>धन्यवाद!</div>
        </div>
      )}
                ...
                {products.map((p: any) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-base font-bold mt-1 text-primary">रू {Number(p.sellingPrice).toLocaleString()}</div>
        ...
        <div className="w-80 shrink-0 flex flex-col bg-card">
                ...
                {cart.map(item => (
                  <div key={item.productId} className="flex items-center gap-2 py-2 border-b border-border/50 last:border-0" data-testid={`cart-item-${item.productId}`}>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground">रू {item.price.toLocaleString()} × {item.quantity}</div>
                    </div>
                    ...
                    <div className="w-18 text-right text-sm font-semibold">रू {(item.price * item.quantity).toLocaleString()}</div>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="border-t border-border p-3 space-y-3">
              ...
              <SelectContent>
                ...
                {customerList.map((c: any) => (
                  <SelectItem key={c.id} value={String(c.id)}>
                    {c.name} {c.memberNumber ? `(${c.memberNumber})` : ""}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
                ...
                <CardContent className="p-2 space-y-1">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs font-medium">{selectedCustomer.memberNumber}</span>
                    </div>
                    <Badge className={`text-[10px] px-1.5 py-0 ${tierConfig[selectedCustomer.membershipTier]?.color ?? ""}`}>
                      {tierConfig[selectedCustomer.membershipTier]?.label ?? "Basic"}
                    </Badge>
                  </div>
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>Points: {(selectedCustomer.loyaltyPoints ?? 0).toLocaleString()}</span>
                    {tierDiscountPct > 0 && (
                      <span className="text-green-700 font-medium">{tierDiscountPct}% discount applied</span>
                    )}
                  </div>
                  {cart.length > 0 && (
                    <div className="text-xs text-blue-600 font-medium">+{pointsToEarn} pts to earn</div>
                  )}
                </CardContent>
              </Card>
            )}
            ...
            <div className="space-y-1 text-sm">
              ...
              {tierDiscount > 0 && (
                <div className="flex justify-between text-green-700"><span>Tier Discount ({tierDiscountPct}%)</span><span>-{fmt(tierDiscount)}</span></div>
              )}
              <div className="flex justify-between text-muted-foreground"><span>Tax</span><span>{fmt(taxTotal)}</span></div>
              <div className="flex justify-between font-bold text-base border-t border-border pt-1"><span>Total</span><span>{fmt(total)}</span></div>
            </div>
            ...
            {paymentMethod === "cash" && (
              <div className="space-y-2">
                <div className="flex gap-1">
                  <Input className="h-8 text-sm font-mono" placeholder="Amount paid (रू)" value={amountPaid} onChange={e => setAmountPaid(e.target.value)} data-testid="input-amount-paid" />
                  {change > 0 && <div className="text-xs text-green-700 font-semibold self-center ml-1 whitespace-nowrap">↩ {fmt(change)}</div>}
                </div>
                ...
                <div className="grid grid-cols-2 gap-1">
                  {[total, Math.ceil(total / 100) * 100, Math.ceil(total / 500) * 500, Math.ceil(total / 1000) * 1000]
                    ...
                    .slice(0, 4)
                    .map(v => (
                      <button key={v} onClick={() => setAmountPaid(v.toFixed(0))} className="h-7 text-xs rounded-md bg-accent hover:bg-accent/80 transition-colors font-medium">रू {v.toLocaleString()}</button>
                    ))}
                </div>
              </div>
            )}
            ...
            <div className="space-y-3 text-sm">
              ...
              {lastSale.customerName && (
                <div className="bg-muted rounded-md p-2 text-xs space-y-0.5">
                  ...
                  <div className="text-muted-foreground">Member: {lastSale.memberNumber ?? "—"}</div>
                  <div className="flex items-center gap-1">
                    <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${tierConfig[lastSale.tier ?? "basic"]?.color ?? ""}`}>
                      {tierConfig[lastSale.tier ?? "basic"]?.label ?? "Basic"}
                    </span>
                    {(lastSale.pointsEarned ?? 0) > 0 && (
                      <span className="text-blue-600 font-medium">+{lastSale.pointsEarned} pts earned</span>
                    )}
                  </div>
                </div>
              )}
              ...
              <div className="space-y-1 text-xs">
                <div className="flex justify-between text-muted-foreground"><span>Subtotal</span><span>{fmt(Number(lastSale.subtotal))}</span></div>
                {Number(lastSale.discount) > 0 && (
                  <div className="flex justify-between text-green-700"><span>Tier Discount ({Number(lastSale.tierDiscountPct).toFixed(0)}%)</span><span>-{fmt(Number(lastSale.discount))}</span></div>
                )}
                <div className="flex justify-between text-muted-foreground"><span>Tax</span><span>{fmt(Number(lastSale.tax))}</span></div>
                <div className="flex justify-between font-bold text-sm"><span>Total</span><span>{fmt(Number(lastSale.total))}</span></div>
                {Number(lastSale.change) > 0 && (
                  <div className="flex justify-between text-green-700 font-medium"><span>Change</span><span>{fmt(Number(lastSale.change))}</span></div>
                )}
              </div>
              ...
              <div className="flex gap-2">
                ...
                <Button className="flex-1" onClick={() => setReceiptOpen(false)} data-testid="button-receipt-close">New Sale</Button>
              </div>
            </div>
          )}
        </DialogContent>
